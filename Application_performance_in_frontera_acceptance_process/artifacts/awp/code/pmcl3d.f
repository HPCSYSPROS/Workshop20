...
C     INTEGRATION/PERFORMANCE/EXTRA VARIABLES    
      ...
      integer :: filetype,filetype2, filetype3, filetype4
      integer :: xn,yn,zn
...
C     ====================================================
      ...
      if(rank==master) then
        ...
        call openpar(1)
C       call openlog(1)
        call rddt3d(tmax,dh,dt,npc,nd,arbc,nsrc,nst,nx,ny,nz,
     ...
     +       mu_ss,mu_dd,fl,
     +       fh,fp,chkp,chkj,insrc,invel,sxgro,sygro,szgro,
     +       sxgro2,sygro2,szgro2,read_step,write_step,write_step2,sgsn,
     +       filen,pht,
     +       ifault,checkpoint,isfcvlm,imd5,ivelocity,
     +       mediarestart,io_opt,iperf,idyna,nvar,iost,partdeg,neg_lambda)
...
C     ==========================
      ...
      call MPI_BCAST(neg_lambda,1,MPI_INTEGER, master,MCW,err)
...
C     ==============================================
      ...
      allocate(tpmap(npts))
      call getrec(coords,maxdim,rank,nxt,nyt,nzt,nz,recproc,nprec,nrec,
     +     nbgx,nedx,nskpx,nbgy,nedy,nskpy,nbgz,nedz,nskpz,1)

      if (isfcvlm==1 .or. ifault==4) then
        allocate(tprec(npts,maxdim))
        allocate(tpmap(npts))
        call getrec(coords,maxdim,rank,nxt,nyt,nzt,nz,recproc2,nprec2,nrec2,
     +       nbgx2,nedx2,nskpx2,nbgy2,nedy2,nskpy2,nbgz2,nedz2,nskpz2,2)
      endif                                        

      if (ifault==3 .or. ifault==4) then
         allocate(tprec(npts,maxdim))
         allocate(tpmap(npts))
         call getrec(coords,maxdim,rank,nxt,nyt,nzt,nz,recproc3,nprec3,nrec3,
...
C     ===================================================
      ...
      if(rank==recproc2) shape2=1
      call MPI_COMM_SPLIT(MC1,shape2,0,MCR2,err)
...
c     ----------------------------------------------
      ...
      if(isfcvlm==1 .or. ifault==4) then
         allocate(tpmap(nprec2*write_step2))
         do i = 1, write_step2
            do j = 1, nprec2
               tpmap((i-1)*nprec2+j) = pmap2(j)+int((i-1)*nrec2,MPI_ADDRESS_KIND)
            end do
         end do
         deallocate(pmap2)
         allocate(pmap2(nprec2*write_step2))
c     ----------------------------------------------
c     use the following line on XT3, BG/L, XeonEM64T
c     ----------------------------------------------
         allocate(block(nprec2*write_step2))
         
         do i = 1, write_step2*nprec2
            pmap2(i) = tpmap(i)
c     ----------------------------------------------
c     use the following line on XT3, BG/L, XeonEM64T
c     ----------------------------------------------
...
c     use the following line on PWR4, IA-64
c     -------------------------------------
c     call MPI_TYPE_CREATE_INDEXED_BLOCK(nprec2*write_step2,1,pmap2,MPI_REAL,
c
c     ---------------------------------------------------------
c     use the following line on XT3, BG/L, XeonEM64T - 32BIT IO
c     ---------------------------------------------------------
c     call MPI_TYPE_INDEXED(nprec2*write_step2,block,pmap2,MPI_REAL,
c
c     ---------------------------------------------------------
c     use the following line on XT3, BG/L, XeonEM64T - 64BIT IO
c     ---------------------------------------------------------
         pmap2=pmap2*4          ! byte addressing instead of element addressing
         call MPI_TYPE_CREATE_HINDEXED(nprec2*write_step2,block,pmap2,MPI_REAL,
c
c     ----------------------------------------------------------
c     each of the 3 options above will use the following line to
c     close off MPI-IO data type creation
c     ----------------------------------------------------------
     +        filetype2,err)
         ...
         call MPI_TYPE_COMMIT(filetype2,err)
         deallocate(pmap2)
c     ----------------------------------------------
c     use the following line on XT3, BG/L, XeonEM64T
c     ----------------------------------------------
...
c     define mpi_subarry ends
      ...
      call vsemem(ifault,nve,nxt,nyt,nzt) 
...
C     ===============================
     ...
     +            taumin,taumax,fl,fh,fp,bsize,y_dt,idyna,nve,mediarestart,nvar,iost,partdeg,neg_lambda)
...
C     ===================================================
      ...
      if(rank==master) then
        ...
        call wrpm3d(nxt,nyt,nzt,nt,dh,dt,npc,
...
C     =======================================
     ...
     +              wu2ed,ed2wu,wd2eu,eu2wd)
           ...
           ! ------------------------- 
           call addsrc(i,dh,dt,nst,npsrc,read_step,ifault,rank)
        endif
      endif
...
C     ----------------------------------
     ...
     +            sygro_md5,szgro_md5,imd5,ifault,1,filen)
        ...
        if ((isfcvlm==1) .or. (ifault==4)) then
          call open3dp2(1,MCR2,fhx2,fhy2,fhz2,sxgro2_timestep,sygro2_timestep,
...
c     if(fsf==1) call fstr(nxt,nyt,nzt)
     ...
     +                 nsrc,nst,npsrc,nz,i/read_step +1,read_step)
               ...
               call addsrc(i+1,dh,dt,nst,npsrc,read_step,ifault,rank)
            endif               !end of (ifault==)
...
!     ------------------------------------------------------
         ...
         if (io_opt == 1) then
            ...
            !                         if ifault = 4
            ! --------------------------------------------------------------------------------            
            if ((isfcvlm==1 .or. ifault==4) .and. rank==recproc2) then
               it = ntiskp2*int(i/ntiskp2)
               if (i==it) then
                  if (count2 == 0) then
                     allocate(bufx2(nprec2*write_step2))
                     allocate(bufy2(nprec2*write_step2))
                     allocate(bufz2(nprec2*write_step2))
                     bufx2=-1.0
                     bufy2=-1.0
                     bufz2=-1.0
                  end if
     ...
     +                       sygro2_md5,szgro2_md5,imd5,ifault,1,filen)
                        ...
                        call MPI_FILE_SET_VIEW(fhx2,disp2,MPI_REAL,filetype2,'native',
     +                       MPI_INFO_NULL,err)
                        
                        call MPI_FILE_SET_VIEW(fhy2,disp2,MPI_REAL,filetype2,'native',
     +                       MPI_INFO_NULL,err)
                        
                        call MPI_FILE_SET_VIEW(fhz2,disp2,MPI_REAL,filetype2,'native',
     +                       MPI_INFO_NULL,err)
                     else
                        call MPI_FILE_SET_VIEW(fhx2,disp2,MPI_REAL,filetype2,'native',
     +                       MPI_INFO_NULL,err)
     ...
     +                       MPI_INFO_NULL,err)
                  ...
                  do j=1,nprec2
                     ...
                     yn = prec2(j,2)
                     zn = prec2(j,3)
                     bufx2(nprec2*(count2-1)+j) = u1(xn,yn,zn)
                     bufy2(nprec2*(count2-1)+j) = v1(xn,yn,zn)
                     bufz2(nprec2*(count2-1)+j) = w1(xn,yn,zn)
                  end do
                  
                  if ((count2 == write_step2) .or. (it==(ntiskp2*int(nt/ntiskp2)))) then
                     t22=mpi_wtime()
                     call MPI_FILE_WRITE_AT_ALL(fhx2,offset,bufx2,nprec2*write_step2,MPI_REAL,status,err)
                     t23=mpi_wtime()
                     call MPI_FILE_WRITE_AT_ALL(fhy2,offset,bufy2,nprec2*write_step2,MPI_REAL,status,err)
                     call MPI_FILE_WRITE_AT_ALL(fhz2,offset,bufz2,nprec2*write_step2,MPI_REAL,status,err)
                     
                     if(ivelocity==1) then
                        call open3dp2(0,MCR2,fhx2,fhy2,fhz2,sxgro2_timestep,sygro2_timestep,szgro2_timestep,
