...
C     SOURCE PROCS, LOCATION AND INITIALIZATION
        ...
        ! CASE 2: partitioned source file
          if ((ifault.lt.2).or.(nst==read_step)) then ! for CASE 0,1 or no time spliting (CASE 0,1,2)
            do cur_step= 1, nst, read_step
              if(rank==master) call rdsrcpos(nsrc,nst,nz,ifault,filen,idyna,read_step,cur_step)
     ...
     +                      nsrc,nst,npsrc,nz,1,read_step)
        ...
        ! CASE 3,4: dynamic mode
        ! IDYNA=1 : SGSN mode
          if (rank==master) call rdsrcpos(nsrc,nst,nz,ifault,filen,idyna,nst,1)
          call MPI_BCAST(tpsrc,nsrc*maxdim,MPI_INTEGER,master,comm,err)
     ...
     +                             nsrc,nst,npsrc,nz,1,read_step)
      ...
      subroutine rdsrcpos(nsrc,nst,nz,ifault,filen,idyna,read_step,cur_step)
CCC   ACQUIRE FAULT NODES AND S(T)
      use parstat
      implicit none
     
C     NUMBER OF FAULT NODES AND RUPTURE FUNCTION STEPS
      integer :: i,j,ifault,nsrc,nst,nz,filen,idyna,read_step,cur_step
...
C ==
      ...
      if ((ifault==1).and.(npsrc.ne.0))  then
        write( faultfile, '(a,i7.7)') 'input_rst/srcpart/part_faults/fault', rank
...
!!    Read axx,...axy from corresponding splitted sub-grid fault file
      ...
      if (npsrc.ne.0)  then
        write(faultfile, '(a,i7.7,a,i3.3)') 'input_rst/srcpart/split_faults/fault', rank, '_', idx
        if (rank==master)  print *,'Read source from ', faultfile,bsize,npsrc,read_step
        open(19, file=faultfile, form='unformatted', access='direct',recl=bsize*npsrc*read_step*3, status='old')
        read(19, rec=1) axx,ayy,azz
        read(19, rec=2) axz,ayz,axy
        close(19)
      end if
      ...
      subroutine addsrc(i,dx,dt,nst,npsrc,read_step,ifault,rank)

CCC   ROUTINE TO ADD STAGGERED-GRID SOURCE TO STRESS TENSOR
      use parstat
      implicit none

      integer :: i,nst,npsrc,read_step,ifault,rank
      real :: dt,dx,vtst
      integer :: j,k
...
!       print *, 'in addsrc, timestep, k, npsrc: ', i, k, npsrc
!       print *, 'dt, dx: ', dt,dx
