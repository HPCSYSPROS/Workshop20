...
CCC
     ...
     +                   fl,fh,fp,bsize,y_dt,idyna,nve,readoption,nvar,iost,partdeg,neg_lambda)
...
CCC   ROUTINE TO INITIALIZE SPACE WITH PREPPED 3D STRUCTURE
      ...
      real :: tmp1, tmp2
      real :: vpvs
      integer :: y_dt,idyna,nve,readoption,neg_lambda
...
!     for option 3 
      ...
      if (rank.eq.0) print *, "inimesh - ", nve,readoption,nvar,neg_lambda     
...
c calculate d1,mu,lam,qp,qs
        do i=1,nxt
          do j=1,nyt
            do k=1,nzt
              ! only if neg_lambda flag is on
              !if (tmpvs(i,j,k).lt.400.) then
              !   tmpvs(i,j,k)=400.
              !   tmpvp(i,j,k)=1200.
              !endif
              if (neg_lambda.eq.1) then
