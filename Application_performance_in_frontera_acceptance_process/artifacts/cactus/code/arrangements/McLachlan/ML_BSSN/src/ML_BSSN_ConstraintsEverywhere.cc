...
extern "C" void ML_BSSN_ConstraintsEverywhere_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ML_BSSN_ConstraintsEverywhere_calc_every != ML_BSSN_ConstraintsEverywhere_calc_offset)
    return;
  CCTK_INT ierr CCTK_ATTRIBUTE_UNUSED = 0;
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_BSSN::ML_cons_detg","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for ML_BSSN::ML_cons_detg.");
  ierr = Boundary_SelectGroupForBC(cctkGH, CCTK_ALL_FACES, GetBoundaryWidth(cctkGH), -1 /* no table */, "ML_BSSN::ML_cons_traceA","flat");
  if (ierr < 0)
    CCTK_WARN(CCTK_WARN_ALERT, "Failed to register flat BC for ML_BSSN::ML_cons_traceA.");
  return;
}

static void ML_BSSN_ConstraintsEverywhere_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(ML_BSSN_ConstraintsEverywhere,
    ...
    vecimin,vecimax, CCTK_REAL_VEC_SIZE)
  {
    ...
    CCTK_REAL_VEC cAL CCTK_ATTRIBUTE_UNUSED = trAt;
    /* Copy local copies back to grid functions */
    vec_store_partial_prepare(i,vecimin,vecimax);
    vec_store_nta_partial(cA[index],cAL);
    vec_store_nta_partial(cS[index],cSL);
  }
  CCTK_ENDLOOP3STR(ML_BSSN_ConstraintsEverywhere);
}
extern "C" void ML_BSSN_ConstraintsEverywhere(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_ConstraintsEverywhere_Body");
  }
  if (cctk_iteration % ML_BSSN_ConstraintsEverywhere_calc_every != ML_BSSN_ConstraintsEverywhere_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "ML_BSSN::ML_curv",
    "ML_BSSN::ML_metric"};
  AssertGroupStorage(cctkGH, "ML_BSSN_ConstraintsEverywhere", 4, groups);
  ...
  LoopOverInterior(cctkGH, ML_BSSN_ConstraintsEverywhere_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_ConstraintsEverywhere_Body");
  }
}
