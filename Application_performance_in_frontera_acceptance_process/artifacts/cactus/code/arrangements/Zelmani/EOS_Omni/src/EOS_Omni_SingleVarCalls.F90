...
subroutine EOS_Omni_EOS_Press(eoskey,keytemp,rf_precision,npoints,&
  ...
  select case (eoskey)
     ...
     case (7)
        ...
        ! Note: We abuse the "hybrid" local variables, 
        ! but note that the pwp vars are not in cgs units. "hybrid_local_k_cgs" therefore is just a name!
        do i=1,npoints
...
subroutine EOS_Omni_EOS_DPressByDRho(eoskey,keytemp,rf_precision,npoints,&
  ...
  character(256)   :: warnstring
  real*8           :: hybrid_local_gamma, hybrid_local_k_cgs, &
                      hybrid_dp_poly, hybrid_dp_th1, hybrid_dp_th2, pwp_a
  real*8,parameter :: zero = 0.0d0
  ! temporary vars for nuc_eos
  real*8           :: xrho,xye,xtemp,xenr,xent
  real*8           :: xprs,xmunu,xcs2
  ...
  select case (eoskey)
     ...
     case (7)
        ...
        do i=1,npoints
           ...
           dpdrhoe(i) = hybrid_local_gamma * hybrid_local_k_cgs * (rho(i))**(hybrid_local_gamma-1.0d0) &
...
subroutine EOS_Omni_EOS_DPressByDEps(eoskey,keytemp,rf_precision,npoints,&
  ...
  select case (eoskey)
     ...
     case (7)
        ! piecwise polytrope hybrid EOS
        do i=1,npoints
           dpdepsrho(i) = (pwp_gamma_th - 1.0d0) * rho(i)
       enddo
  ...
  select case (eoskey)
     ...
     case (7)
        ...
        do i=1,npoints
           ...
           cs2(i) = (xdpdrhoe + xpress*xdpderho/(rho(i)**2)) / (1.0d0 + eps(i) + xpress/rho(i))
       enddo
...
subroutine EOS_Omni_EOS_temp_from_press(eoskey,keytemp,rf_precision,npoints,&
                              rho,eps,temp,ye,press,xeps,keyerr,anyerr)
  ...
  select case (eoskey)
     case (1)
        ! polytropic EOS
        write(warnstring,*) "EOS_Omni_EpsFromPress call not supported for polytropic EOS"
        call CCTK_ERROR(warnstring)
        STOP
     case (2)
        ! gamma-law EOS
        write(warnstring,*) "EOS_Omni_EpsFromPress call not supported for gamma-law EOS"
        call CCTK_ERROR(warnstring)
        STOP
     case (3)
        ! hybrid EOS
     ...
     case (4)
        ...
        if(keytemp.eq.0) then
           call nuc_eos_m_kt0_press_cs2(npoints,rho,temp,ye,&
                eps,xprs,xcs2,rf_precision,keyerr,anyerr)
        else
           call CCTK_ERROR("This keytemp is not supported!")
           STOP
        endif
...
end subroutine EOS_Omni_EOS_temp_from_press
...
!        STOP
        ...
        do i=1,npoints
           ...
           do while(cont)
              ...
              call nuc_eos_m_kt1_short(1,rho_guess,xtemp,xye,xenr,&
                   press_guess,xent,xcs2,xdedt,xdpderho,xdpdrhoe,xmunu,&
              ...
              if (rho_guess.lt.1.0d-15.or.counter.gt.100000) then
                 !$OMP CRITICAL
                 write(warnstring,"(1P10E15.6)") rho_guess,xtemp,xye
                 call CCTK_ERROR(warnstring)
                 !$OMP END CRITICAL                 
                 keyerr(i) = 473
                 anyerr = 1
