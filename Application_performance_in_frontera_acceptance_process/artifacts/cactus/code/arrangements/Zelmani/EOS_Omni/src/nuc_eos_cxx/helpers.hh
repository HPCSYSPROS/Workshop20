  ...
  static inline __attribute__((always_inline))
    ...
    *keyerrt = 0;
...
#endif
    if(CCTK_BUILTIN_EXPECT(fabs(press-press0) < prec*fabs(press0), false)) {
      *ltout = lt0;
      return;
    }
	...
	press2 = linterp2D(&logrho[irho-1],&yes[iye-1], fs, lr, ye);
      ...
      // Check if we are already bracketing the input internal
      // energy. If so, interpolate for new T.
      if(CCTK_BUILTIN_EXPECT((press0 - press1) * (press0 - press2) <= 0., false)) {
	...
	*ltout = (logtemp[itemp]-logtemp[itemp-1]) / (press2 - press1) * 
	...
	return;
      ...
      if(CCTK_BUILTIN_EXPECT(err < prec*fabs(press0), false)) {
	*ltout = lt;
	return;
      }
