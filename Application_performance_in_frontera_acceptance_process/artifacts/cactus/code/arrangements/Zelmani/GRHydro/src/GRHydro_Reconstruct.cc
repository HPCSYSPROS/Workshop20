      ...
      for (int k=GRHydro_stencil-1; k < cctk_lsh[2] - GRHydro_stencil+1+transport_constraints*(1-*zoffset); ++k)
         for (int j=GRHydro_stencil-1; j < cctk_lsh[1] - GRHydro_stencil+1+transport_constraints*(1-*yoffset); ++j)
            for (int i=GRHydro_stencil-1; i < cctk_lsh[0] - GRHydro_stencil+1+transport_constraints*(1-*xoffset); ++i) {
               ...
               int extrap_ijk = 0; int extrap_ijk_p = 0; int extrap_ijk_m = 0;
               ...
               if (!excise && hydro_excision_mask[ijk_m]) {
                  extrapolate = true;
                  extrap_ijk = ijk_m;
                  // this is in case we do Wv reconstruction
                  extrap_ijk_m = CCTK_GFINDEX3D(cctkGH, i-*xoffset*2, j-*yoffset*2, k-*zoffset*2);
                  extrap_ijk_p = ijk;
               }
               ...
               if (!excise && hydro_excision_mask[ijk_p]) {
                  ...
                  // this is in case we do Wv reconstruction
                  extrap_ijk_m = ijk;
                  extrap_ijk_p = CCTK_GFINDEX3D(cctkGH, i+*xoffset*2, j+*yoffset*2, k+*zoffset*2);
               }
               ...
               if (extrapolate)
               {
                  ...
                  if (reconstruct_Wv) {
                     velxminus[extrap_ijk] = w_lorentz[ijk]*velx[ijk];
                     velxplus[extrap_ijk]  = w_lorentz[ijk]*velx[ijk];
                     velyminus[extrap_ijk] = w_lorentz[ijk]*vely[ijk];
                     velyplus[extrap_ijk]  = w_lorentz[ijk]*vely[ijk];
                     velzminus[extrap_ijk] = w_lorentz[ijk]*velz[ijk];
                     velzplus[extrap_ijk]  = w_lorentz[ijk]*velz[ijk];
                 
                     undo_Wv_ptw(gxx, gxy, gxz, gyy, gyz, gzz,
                             velxplus, velyplus, velzplus, extrap_ijk, extrap_ijk_p);
                     undo_Wv_ptw(gxx, gxy, gxz, gyy, gyz, gzz,
                             velxminus, velyminus, velzminus, extrap_ijk, extrap_ijk_m);
                  } else {
