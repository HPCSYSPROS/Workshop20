...
void bisection2(double lr, double lt0, double ye, double leps0, double* ltout,
...
//  printf("------------------------------------\n");
//  printf("LTMAX %16.15e %16.15e %16.15e\n",lt,lt1,lr);
//  printf("ISSUE WITH %d %d %d?\n",struct_ptr->nrho,struct_ptr->ntemp,struct_ptr->nye);
...
//  printf("------------------------------------\n");
  ...
  while(it < itmax) {
    ...
    // upper temperature
    {
      ...
      double fs[4];
      // point 0
      int ifs = 1 + NSUBTABLES*(irho-1 + struct_ptr->nrho*((itemp) + struct_ptr->ntemp*(iye-1)));
      fs[0] = struct_ptr->fourtables[ifs];
      // point 1 
...
// #endif
    bisection2(lr, lt0, ye, leps0, ltout, 1, prec, keyerrt,struct_ptr);
// #if DEBUG
...
void bisection2(double lr, double lt0, double ye, double leps0, double* ltout,
  ...
  int nvars = 1;
  int ivs[1] = {iv};
  nuc_eosK_C_linterp_some(lr, lt1, ye, &f1a, struct_ptr->fourtables, 
  			 ivs, struct_ptr->nrho, struct_ptr->ntemp, struct_ptr->nye, nvars,
  			 struct_ptr->logrho, struct_ptr->logtemp, struct_ptr->yes, struct_ptr);

  nuc_eosK_C_linterp_some(lr, lt2, ye, &f2a, struct_ptr->fourtables, 
			 ivs, struct_ptr->nrho, struct_ptr->ntemp, struct_ptr->nye, nvars,
  			 struct_ptr->logrho, struct_ptr->logtemp, struct_ptr->yes, struct_ptr);
  ...
  while(f1*f2 >= 0.0) {
    ...
    while(f1a < f2a && ifixdeg < ifixdeg_max) {
      lt1 = log10( MIN(pow(10.0,ltmax),1.2*(pow(10.0,lt1))) );
      nuc_eos_C_linterp_some(lr, lt1, ye, &f1a, fourtables, 
...
// #endif
  ...
  for(it=0;it<itmax;it++) {
    ...
    nuc_eosK_C_linterp_some(lr, ltmid, ye, &f2a, struct_ptr->fourtables, 
			   ivs, struct_ptr->nrho, struct_ptr->ntemp, struct_ptr->nye, nvars,
			   struct_ptr->logrho, struct_ptr->logtemp, struct_ptr->yes,struct_ptr);
