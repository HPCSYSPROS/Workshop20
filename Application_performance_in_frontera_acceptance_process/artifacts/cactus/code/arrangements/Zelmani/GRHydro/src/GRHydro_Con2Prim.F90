...
subroutine Conservative2Primitive(CCTK_ARGUMENTS)
  ...
  CCTK_REAL :: s2,corrfac
  CCTK_REAL :: press_save,rho_save,eps_save
...
! end EOS Omni vars
  ...
  !$OMP warnline, dummy1, dummy2, corrfac, s2, xtemp, xye, xeps, press_save,&
  !$OMP rho_save, eps_save, local_perc_ptol)
  do k = 1, nz 
    do j = 1, ny 
      do i = 1, nx

         ! IMPORTANT NOTE: we need to do con2prim even also in not-evolved
         ! zones: namely in buffer zones and boundary zones so that we have
         ! consistent boundary information. So it is not a good idea
         ! to use the evolution mask in con2prim
         !
         ! do not do con2prim if this point is not evolved on this grid

         !do not compute if in atmosphere region!
         if (atmosphere_mask(i,j,k) .gt. 0) then
            cycle
         endif
...
!        if ((hydro_excision_mask(i,j,k) .gt. 0)) then
        ...
        if(evolve_Y_e.ne.0) then
         ...
         ! Faber et al. 2007 checks to get sane values of the
         ! conserved variables near punctures
         if(hydro_excision_mask(i,j,k).gt.0) then
            ...
            if(tau(i,j,k).lt.0.0d0) then 
               ! If tau < 0, reset it to sane values as proposed by 
               ! Faber, Etienne, Farris (Illinois)
               tau(i,j,k) = GRHydro_tau_min_fac * GRHydro_tau_max
            endif
            
            if(s2.ge.tau(i,j,k)*(tau(i,j,k) + 2.0d0 * dens(i,j,k))) then
               corrfac = sqrt(0.98d0 * tau(i,j,k) * &
                    (tau(i,j,k) + 2.0d0 * dens(i,j,k)) / s2)
               scon(i,j,k,1) = scon(i,j,k,1) * corrfac
               scon(i,j,k,2) = scon(i,j,k,2) * corrfac
               scon(i,j,k,3) = scon(i,j,k,3) * corrfac
               local_perc_ptol = GRHydro_perc_ptol
            else
               local_perc_ptol = GRHydro_perc_ptol
            endif
         endif
         ! end Faber et al. 2007 checks


         if(evolve_temper.eq.0) then
            ! save key old primitives
            press_save = press(i,j,k)
            rho_save = rho(i,j,k)
            eps_save = eps(i,j,k)
            ...
            if(abs(GRHydro_C2P_failed(i,j,k)).gt.0.0d0) then
               ! okay, con2prim failed. Let's be a little liberal
               ! and apply the Faber+07 fixes even outside the horizon

               press(i,j,k) = press_save
               eps(i,j,k) = eps_save
               rho(i,j,k) = rho_save
               GRHydro_C2P_failed(i,j,k) = 0.0d0
               s2 = uxx*scon(i,j,k,1)**2 + uyy*scon(i,j,k,2)**2 &
                    + uzz*scon(i,j,k,3)**2 &
                    + 2.0d0*uxy*scon(i,j,k,1)*scon(i,j,k,2) &
                    + 2.0d0*uxz*scon(i,j,k,1)*scon(i,j,k,3) & 
               ...
               ! if bisection fails, then God help us
               call Con2Prim_pt_bisect(int(cctk_iteration,ik),int(i,ik),int(j,ik),int(k,ik),&
                    GRHydro_eos_handle, dens(i,j,k),scon(i,j,k,1),scon(i,j,k,2), &
                    scon(i,j,k,3),tau(i,j,k),rho(i,j,k),vup(i,j,k,1),vup(i,j,k,2), &
                    vup(i,j,k,3),eps(i,j,k),press(i,j,k),w_lorentz(i,j,k), &
                    uxx,uxy,uxz,uyy,uyz,uzz,sdetg(i,j,k),x(i,j,k),y(i,j,k), &
                    z(i,j,k),r(i,j,k),epsnegative,GRHydro_rho_min,pmin, epsmin, & 
                    GRHydro_reflevel, GRHydro_C2P_failed(i,j,k), local_perc_ptol, myproc)
...
!!$Find the root
  ...
  do while ( ((abs(pnew - pold)/abs(pnew) .gt. GRHydro_perc_ptol) .and. &
...
#if 0
    if(cctk_iteration.eq.8577.and.GRHydro_reflevel.eq.6.and.&
         ii.eq.14.and.jj.eq.13.and.kk.eq.6) then
       if (count > GRHydro_countmax/2) then
          mustbisect = .true.
          write(warnline,"(A3,i5,1P10E15.6)") "X1*",count,pnew,pold,abs(pnew - pold)/abs(pnew)
          call CCTK_WARN(1, warnline)
       endif
    endif
#endif
    ...
    if (count > GRHydro_countmax) then
      !$OMP CRITICAL
      call CCTK_WARN(1, 'count > GRHydro_countmax! Switching to bisection routine!')
      write(warnline,"(i8,i4,3i5)") cctk_iteration,GRHydro_reflevel,ii,jj,kk
      call CCTK_WARN(1, warnline)
      !$OMP END CRITICAL
      GRHydro_C2P_failed = 666.0d0
      return
    end if
      ...
      do i = 1, nx
        ...
        if (GRHydro_C2P_failed(i,j,k) .ge. 1) then
          ...
          write(warnline,'(a32,3g16.7)') 'xyz location: ',&
               x(i,j,k),y(i,j,k),z(i,j,k)
          call CCTK_WARN(1,warnline)
          write(warnline,'(a32,3i4)') 'ijk location: ',&
               i,j,k
          call CCTK_WARN(1,warnline)
          write(warnline,'(a32,i8)') 'cctk_iteration: ',&
               cctk_iteration
          call CCTK_WARN(1,warnline)
