...
int Einstein_RegisterSlicing(const char *slice_name) {
  ...
  if (handle < 0) {
    ...
    new_slicing =
        (struct Einstein_slicing *)malloc(sizeof(struct Einstein_slicing));
    ...
    CCTK_VWarn(0, __LINE__, __FILE__, CCTK_THORNSTRING,
               "New handle %s (%d) already in use", slice_name, handle);
    handle = -1;
  }
...
int Einstein_RegisterTimeToSlice(int handle, int (*func)(cGH *)) {
  ...
  if (slicing) {
    ...
    CCTK_VWarn(1, __LINE__, __FILE__, CCTK_THORNSTRING,
               "Could not obtain slicing structure for handle %d", handle);
    return_code = 0;
  }
  ...
  *active_slicing_handle = -1;
  ...
  if (!CCTK_Equals(lapse_evolution_method, "coordgauge")) {
    ...
    if (slic->timetoslice != NULL) {
      ...
      sprintf(err, "%s%s%s%s", "ERROR: You have specified a unique slicing (",
              slic->name,
              ") AND a condition-function is registered for that slicing.\n ",
              "Impossible, what should I do inbetween ? Rather set slicing -> "
              "mixed");
      CCTK_WARN(0, err);
      if (err)
        free(err);
  ...
  /* We have mixed slicing: we check the mixed_slicing parameter */
    ...
    */
    ...
    while (item) {
      ...
      slic =
          Util_GetHandledData(Eslicings, Util_GetHandle(Eslicings, item, NULL));
...
void Einstein_SetNextSlicing(cGH *GH) {
  ...
  int h, doicare = 0;
  int h_yes = num_slicings, h_no = num_slicings, h_egal = num_slicings;
  ...
  if (!CCTK_Equals(lapse_evolution_method, "coordgauge")) {
    ...
    if (h < 0) {
      CCTK_WARN(
          0, "Slicing parameter specifies non-activated/registered slicing!");
    }
      ...
      if (!slic) {
        ...
        */
        ...
        printf(" handle:%d name:%s param_active:%d h_yes:%d h_egal:%d \n", h,
               slic->name, slic->param_active, h_yes, h_egal);
#endif
      }
    }
  }
  ...
  if (CCTK_Equals(slicing_verbose, "yes")) {
    ...
    slic = (struct Einstein_slicing *)Util_GetHandledData(
        Eslicings, *active_slicing_handle);
    if (!slic) {
      CCTK_WARN(0, "Cannot find slicing handle");
    }

    sprintf(info, "Next slicing: %s (%d)", slic->name,
            (int)*active_slicing_handle);
    CCTK_INFO(info);
    if (info)
      free(info);
  }
