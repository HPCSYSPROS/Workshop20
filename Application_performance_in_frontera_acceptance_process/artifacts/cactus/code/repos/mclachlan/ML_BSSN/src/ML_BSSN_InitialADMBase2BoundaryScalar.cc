...
extern "C" void ML_BSSN_InitialADMBase2BoundaryScalar_SelectBCs(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  
  if (cctk_iteration % ML_BSSN_InitialADMBase2BoundaryScalar_calc_every != ML_BSSN_InitialADMBase2BoundaryScalar_calc_offset)
    return;
...
static void ML_BSSN_InitialADMBase2BoundaryScalar_Body(const cGH* restrict const cctkGH, const int dir, const int face, const CCTK_REAL normal[3], const CCTK_REAL tangentA[3], const CCTK_REAL tangentB[3], const int imin[3], const int imax[3], const int n_subblock_gfs, CCTK_REAL* restrict const subblock_gfs[])
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  #pragma omp parallel
  CCTK_LOOP3STR(ML_BSSN_InitialADMBase2BoundaryScalar,
  ...
  CCTK_ENDLOOP3STR(ML_BSSN_InitialADMBase2BoundaryScalar);
}
extern "C" void ML_BSSN_InitialADMBase2BoundaryScalar(CCTK_ARGUMENTS)
{
  DECLARE_CCTK_ARGUMENTS;
  DECLARE_CCTK_PARAMETERS;
  ...
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Entering ML_BSSN_InitialADMBase2BoundaryScalar_Body");
  }
  if (cctk_iteration % ML_BSSN_InitialADMBase2BoundaryScalar_calc_every != ML_BSSN_InitialADMBase2BoundaryScalar_calc_offset)
  {
    return;
  ...
  const char* const groups[] = {
    ...
    "ML_BSSN::ML_dtshift",
    "ML_BSSN::ML_Gamma"};
  AssertGroupStorage(cctkGH, "ML_BSSN_InitialADMBase2BoundaryScalar", 3, groups);
  ...
  LoopOverBoundary(cctkGH, ML_BSSN_InitialADMBase2BoundaryScalar_Body);
  if (verbose > 1)
  {
    CCTK_VInfo(CCTK_THORNSTRING,"Leaving ML_BSSN_InitialADMBase2BoundaryScalar_Body");
  }
}
