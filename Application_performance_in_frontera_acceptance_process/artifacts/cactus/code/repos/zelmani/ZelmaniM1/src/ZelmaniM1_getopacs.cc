...
#pragma omp for
      for(int k=0;k<nz;k++) 
          ...
          for(int i=0;i<nx;i++) 
	    {
	      ...
	      gamma.loadLapseShift(alp[i3D],betax[i3D]);
		...
		if (isnan(em))
	          CCTK_VInfo(CCTK_THORNSTRING, "bad emis: %d %d %d %d %14.5e %14.5e %14.5e %14.5e",
		                                i,j,k,ig,pow(10.0,xrho),pow(10.0,xtemp),Y_e[i3D],em);
		if (isnan(ab))
		  CCTK_VInfo(CCTK_THORNSTRING, "bad abs : %d %d %d %d %14.5e %14.5e %14.5e %14.5e",
		...
		double dtau = 1.e15;
	        ...
	        cl.setClosure(0.5*(xioldzL[i4D]+xioldzR[i4D]));
		...
		fnuzh[i4D] = fnuz[i4D];
		if (do_m1_nevolve) {
		  nnu[i4D]   = max(N*free_stream_fac,1.e-40);
		  nnuh[i4D]  = nnu[i4D];
		}
		
		if (do_m1_pnu) pnu[i3D] += E*free_stream_fac/3.0;
...
#pragma omp for
      for(int k=0;k<nz;k++) 
		...
		if (absorb[ind4D] <= 0.0 || isnan(absorb[ind4D])) CCTK_VInfo(CCTK_THORNSTRING, "bad abs : %d %d %d %d %14.5e %14.5e %14.5e %14.5e",i,j,k,ig,pow(10.0,xrho),pow(10.0,xtemp),Y_e[ind3D],absorb[ind4D]);
		if (emis[ind4D] <= 0.0 || isnan(emis[ind4D]))     CCTK_VInfo(CCTK_THORNSTRING, "bad emis: %d %d %d %d %14.5e %14.5e %14.5e %14.5e",i,j,k,ig,pow(10.0,xrho),pow(10.0,xtemp),Y_e[ind3D],emis[ind4D]);
		if (scat[ind4D] <= 0.0 || isnan(scat[ind4D]))     CCTK_VInfo(CCTK_THORNSTRING, "bad scat: %d %d %d %d %14.5e %14.5e %14.5e %14.5e",i,j,k,ig,pow(10.0,xrho),pow(10.0,xtemp),Y_e[ind3D],scat[ind4D]);
#endif
	      }
...
//    const int nz = cctk_lsh[2];
//
//    CCTK_Info(CCTK_THORNSTRING, "Calculate very approximate gray opacs");
//    // timer
//    clock_t start, end;
//    double elapsed;
//    start = clock();
...
//          {
//            int ind3D = CCTK_GFINDEX3D(cctkGH,i,j,k);
//            double nb   = rho[ind3D] * INV_RHO_GF * 6.02e23;
//            double temp = temperature[ind3D];
//            double ye   = Y_e[ind3D];
//            double eta = 0.0;
//
...
//        	{
//                  int ig      = m + l*ngroups; // This indexing may be backward
//                  //int index4D = ig + ngroups*nspecies*CCTK_GFINDEX3D(cctkGH,i,j,k); 
//                  int index4D = CCTK_VECTGFINDEX3D(cctkGH,i,j,k,ig); 
//
//        	  double eeff = temp*sqrt(Fermi_4(eta)/Fermi_2(eta));
//
//        	  // Absorbption rate on neutrons
//        	  // [Msun^-1]
//        	  absorb[index4D] = (1.0-ye)*nb*SIGMA0*3.8296*eeff*eeff;
//        	  // Emissivity for zero chemical potential
//        	  // [MeV fm^-3 Msun^-1]
//        	  emis[index4D]   = absorb[index4D]*C_L*3.7464e-8*pow(temp,4)*Fermi_3(eta)/4.0/3.14159265359;
//        	  // Very Approximate nucleon scattering
//        	  // [Msun^-1]
//        	  scat[index4D]   = 0.346*nb*SIGMA0*3.8296*eeff*eeff; 
//                }
//          }
//
//    end = clock();
//    elapsed = ((double) (end - start)) / CLOCKS_PER_SEC;
//    CCTK_VInfo (CCTK_THORNSTRING,"time used in proc 0: %5.2f",elapsed);
//    CCTK_Info(CCTK_THORNSTRING, "Done calculating gray opacs");
//
//
//    return;
//  }
