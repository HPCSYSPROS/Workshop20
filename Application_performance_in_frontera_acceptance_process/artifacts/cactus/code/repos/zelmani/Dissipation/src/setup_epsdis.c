...
#pragma omp parallel for
      for (int k=0;k<nk;k++) {
        for (int j=0;j<nj;j++) {
          for (int i=0;i<outer_bound_npoints;i++) {
            int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
            int indexP= CCTK_GFINDEX3D(cctkGH,outer_bound_npoints,j,k); 
...
#pragma omp parallel for
      for (int k=0;k<nk;k++) {
        for (int j=0;j<nj;j++) {
          for (int i=ni-1;i>=ni-outer_bound_npoints;i--) {
            int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
            int indexP= CCTK_GFINDEX3D(cctkGH,ni-outer_bound_npoints-1,j,k); 
	    epsdisA[index]=epsdis+ob_slope*fabs(x[index]-x[indexP]);
	    if (epsdisA[index] > outer_boundary_max_epsdis) {
	      epsdisA[index] = outer_boundary_max_epsdis;
	    }
...
#pragma omp parallel for
      for (int k=0;k<nk;k++) {
        for (int j=0;j<outer_bound_npoints;j++) {
          for (int i=0;i<ni;i++) {
            int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
            int indexP= CCTK_GFINDEX3D(cctkGH,i,outer_bound_npoints,k); 
...
#pragma omp parallel for
      for (int k=0;k<nk;k++) {
        for (int j=nj-1;j>=nj-outer_bound_npoints;j--) {
          for (int i=0;i<ni;i++) {
            int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
            int indexP= CCTK_GFINDEX3D(cctkGH,i,nj-outer_bound_npoints-1,k); 
            epsdisA[index]=epsdis+ob_slope*fabs(y[index]-y[indexP]);
	    if (epsdisA[index] > outer_boundary_max_epsdis) {
	      epsdisA[index] = outer_boundary_max_epsdis;
	    }
    ...
    if(doBC[4]) {
      for (int k=0;k<outer_bound_npoints;k++) {
#pragma omp parallel for
        for (int j=0;j<nj;j++) {
          for (int i=0;i<ni;i++) {
            int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
            int indexP= CCTK_GFINDEX3D(cctkGH,i,j,outer_bound_npoints); 
    ...
    if(doBC[5]) {
      for (int k=nk-1;k>=nk-outer_bound_npoints;k--) {
#pragma omp parallel for
        for (int j=0;j<nj;j++) {
          for (int i=0;i<ni;i++) {
            int index = CCTK_GFINDEX3D(cctkGH,i,j,k);
            int indexP= CCTK_GFINDEX3D(cctkGH,i,j,nk-outer_bound_npoints-1); 
            epsdisA[index]=epsdis+ob_slope*fabs(z[index]-z[indexP]);
	    if (epsdisA[index] > outer_boundary_max_epsdis) {
	      epsdisA[index] = outer_boundary_max_epsdis;
	    }
      ...
      for (int i=2;i<ni-2;i++)
       for (int j=2;j<nj-2;j++)
        for (int k=2;k<nk-2;k++) {
          int m=CCTK_GFINDEX3D(cctkGH,i,j,k);
...
#pragma omp parallel for
      for (int i=0;i<npts;i++) {
        double radp=sqrt((xa[i]-odx)*(xa[i]-odx)+(ya[i]-ody)*(ya[i]-ody)+
			 (za[i]-odz)*(za[i]-odz));
