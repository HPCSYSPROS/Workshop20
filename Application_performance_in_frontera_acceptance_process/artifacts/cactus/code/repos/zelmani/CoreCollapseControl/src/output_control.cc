...
void CoreCollapseControl_OutputControl(CCTK_ARGUMENTS)
{
  ...
  if(output_control) {
	...
	CCTK_VInfo(CCTK_THORNSTRING,"CarpetIOHDF5 3D output every %d time steps",every);
      ...
      if(preb_out2Dhdf5_every != -2) {
	int every=preb_out2Dhdf5_every;
	outevery.str("");
	outevery << every;
	
	error += CCTK_ParameterSet("out2D_every","CarpetIOHDF5",
				  outevery.str().c_str());
	
	assert(error==0);
	CCTK_VInfo(CCTK_THORNSTRING,"CarpetIOHDF5 2D output every %d time steps",every);
      }
