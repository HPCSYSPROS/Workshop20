...
subroutine num_x_fluxAM(dens,sx,sy,sz,tau,Bx,By,Bz,&
...
!!$ In the notation of Anton et al.:  [psi^6 D] vtilde^i
  ...
  sxf = sx*vxt+psipstar-bsubx*sdet*Bx/w

  syf = sy*vxt-bsuby*sdet*Bx/w

  szf = sz*vxt-bsubz*sdet*Bx/w

!!$ [psi^6 tau] vtilde^i +p* v^i - alp b^0 B^i/w
  tauf = tau*vxt + psipstar*velm - ab0*sdet*Bx/w

!!$ [psi^6 (B^k vtilde^i - B^i vtilde^k)]
  bxf = 0.0
  byf = sdet*By * vxt - sdet*Bx*vyt
  bzf = sdet*Bz * vxt - sdet*Bx*vzt

end subroutine num_x_fluxAM
