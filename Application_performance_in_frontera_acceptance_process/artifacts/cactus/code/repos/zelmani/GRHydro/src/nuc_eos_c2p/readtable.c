...
//void nuc_eosK_C_ReadTable(char* nuceos_table_name)
void nuc_eosK_C_ReadTable(CCTK_ARGUMENTS)
{
  ...
  fprintf(stdout,"*******************************\n");
  fprintf(stdout,"Reading nuc_eosK table file:\n");
  fprintf(stdout,"%s\n",nuceos_table_name);
  fprintf(stdout,"*******************************\n");
...
#define READ_EOSTABLE_HDF5(NAME,OFF)                                     \
  ...
  if (!(fourtables = (double*)malloc(nrho * ntemp * nye * NSUBTABLES * sizeof(double)))) {
    fprintf(stderr, "Cannot allocate memory for EOS table\n");
    abort();
  }
  int iv,k,j,i;
  for(iv = 0;iv<NTABLES;iv++) 
    for(k = 0; k<nye;k++) 
      for(j = 0; j<ntemp; j++) 
	for(i = 0; i<nrho; i++) {
	  int indold = i + nrho*(j + ntemp*(k + nye*iv));
	  int indnew = iv + NTABLES*(i + nrho*(j + ntemp*k));
	  int indnewFOUR = NSUBTABLES*(i + nrho*(j + ntemp*k));
	  alltables[indnew] = alltables_temp[indold];
	  if(iv==0) //logpress
	    fourtables[0+indnewFOUR] = alltables[indnew];
	  if(iv==1) //logenergy
	    fourtables[1+indnewFOUR] = alltables[indnew];
	  if(iv==5) //dedt
	    fourtables[2+indnewFOUR] = alltables[indnew];
	  if(iv==6) //dpdrhoe
	    fourtables[3+indnewFOUR] = alltables[indnew];
	}
