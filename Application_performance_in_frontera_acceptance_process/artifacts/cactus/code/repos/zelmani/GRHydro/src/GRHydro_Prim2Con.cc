...
static inline void GRHydro_SpeedOfSound(CCTK_ARGUMENTS)
{
   ...
   // depending on whether pressure or eps/temperature was reconstructed, we
   // have to recompute the other quantitites
   void (*eos_post_reconstruct)(CCTK_INT eos_handle, CCTK_INT keytemp, CCTK_REAL rf_prec,
                                CCTK_INT npoints, const CCTK_REAL* rho, CCTK_REAL* eps,
                                CCTK_REAL* temp, const CCTK_REAL* y_e,
                                CCTK_REAL* press, CCTK_REAL* cs2, CCTK_INT* keyerr,
                                CCTK_INT* anyerr) = reconstruct_pressure ?
      NULL : EOS_Omni_press_cs2;
...
#pragma omp parallel for
       for(int k=0;k<cctk_lsh[2];k++)
         for(int j=0;j<cctk_lsh[1];j++) {
           ...
           eos_post_reconstruct(*GRHydro_eos_handle,keytemp,GRHydro_eos_rf_prec,cctk_lsh[0],
...
#pragma omp parallel for
       for(int k=GRHydro_stencil-1;k<cctk_lsh[2]-GRHydro_stencil+1 + (transport_constraints && !*zoffset);k++)
         for(int j=GRHydro_stencil-1;j<cctk_lsh[1]-GRHydro_stencil+1 + (transport_constraints && !*yoffset);j++) {
           ...
           eos_post_reconstruct(*GRHydro_eos_handle,keytemp,GRHydro_eos_rf_prec,nx,
...
#pragma omp critical
                 {
                   ...
                   if(keyerr[idx] == 668) {
                     ...
                     CCTK_INT lanyerr = 0;
                     CCTK_INT lkeytemp = 1;
                     eos_post_reconstruct(*GRHydro_eos_handle,lkeytemp,GRHydro_eos_rf_prec,ln,
