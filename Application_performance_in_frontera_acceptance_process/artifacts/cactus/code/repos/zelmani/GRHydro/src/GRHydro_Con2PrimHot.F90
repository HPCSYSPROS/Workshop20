...
! end EOS Omni vars
  ...
  j = 3
  k = 3
  write(warnline,"(A10,i7,4i5,1P10E15.6)") "info1",&
       cctk_iteration,GRHydro_Reflevel,&
       i,j,k,rho(i,j,k),&
       eps(i,j,k),temperature(i,j,k),y_e(i,j,k)
  call CCTK_WARN(1,warnline)

  write(warnline,"(A10,i7,4i5,1P10E15.6)") "info 2:",&
       cctk_iteration,GRHydro_Reflevel,&
       i,j,k,dens(i,j,k),scon(i,j,k,:),tau(i,j,k)
  call CCTK_WARN(1,warnline)
#endif
  ...
  !$OMP warnline, dummy1, dummy2, reset_to_atmo, old_temp)
  do k = 1, nz 
    do j = 1, ny 
      do i = 1, nx
        ...
        sdet = sdetg(i,j,k)
#if 0
        if(sdet.ne.sdet) then
           !$OMP CRITICAL
           write(warnline,"(4i3,1P20E15.6)") GRHydro_reflevel,i,j,k,&
                gxx(i,j,k),gxy(i,j,k),gxz(i,j,k),gyy(i,j,k),gyz(i,j,k),&
                gzz(i,j,k),alp(i,j,k),betax(i,j,k),betay(i,j,k),betaz(i,j,k),&
                dens(i,j,k),tau(i,j,k),scon(i,j,k,:),rho(i,j,k)
        ...
        if (reset_to_atmo .gt. 0 .or. (GRHydro_enable_internal_excision /= 0 &
             .and. hydro_excision_mask(i,j,k) .gt. 0)) then
          ...
          old_temp = temperature(i,j,k)
          call Con2Prim_pt_hot3(int(cctk_iteration,ik),myproc,int(i,ik),int(j,ik),&
               int(k,ik),GRHydro_eos_handle,&
             ...
             if( abs(GRHydro_C2P_failed(i,j,k)-2.0d0) .lt. 1.0d-10) then
                ...
                if (reflevel.ge.GRHydro_c2p_warn_from_reflevel) then
                   ...
                   write(warnline,"(1P10E15.6)") rho(i,j,k),dens(i,j,k),eps(i,j,k),&
                        temperature(i,j,k),y_e(i,j,k), sdetg(i,j,k)
                   call CCTK_WARN(1,warnline)
                   call CCTK_ERROR("Aborting!!!")
                   STOP
                endif
          ...
          if( abs(GRHydro_C2P_failed(i,j,k)-3.0d0) .lt. 1.0d-10 .or. &
             ...
             if (reflevel.ge.GRHydro_c2p_warn_from_reflevel.and.&
                abs(GRHydro_C2P_failed(i,j,k)-3.0d0) .lt. 1.0d-10) then
                !$OMP CRITICAL
                write(warnline,"(A14,i5,F5.1,1P10E15.6)") "C2P failure:",&
                     reflevel, GRHydro_C2P_failed(i,j,k), GRHydro_hot_atmo_temp, &
                     old_temp, temperature(i,j,k),temperature(i,j,k)-GRHydro_hot_atmo_temp
                call CCTK_WARN(1,warnline)
                ...
                write(warnline,"(A10,i7,4i5,1P10E15.6)") "reset T:",&
                     cctk_iteration,GRHydro_Reflevel,&
                     i,j,k,dens(i,j,k),&
                     scon(i,j,k,:), tau(i,j,k)
                call CCTK_WARN(1,warnline)
                write(warnline,"(A10,i7,4i5,1P10E15.6)") "reset T:",&
                     cctk_iteration,GRHydro_Reflevel,&
                     i,j,k,sdetg(i,j,k),gxx(i,j,k),kxx(i,j,k)
                call CCTK_WARN(1,warnline)
                write(warnline,"(A10,i7,4i5,1P10E15.6)") "reset T:",&
                     cctk_iteration,GRHydro_Reflevel,&
...
!!$  Undensitize the variables 
  ...
  usz = sz * invsdet
  utau = tau * invsdet
  s2 = usx*usx*uxx + usy*usy*uyy + usz*usz*uzz + 2.0d0*usx*usy*uxy + &
...
#if 0
  if(ii.eq.3.and.jj.eq.3.and.kk.eq.3) then
     write(6,"(1P10E15.6)") rho, w_lorentz, epsilon, temp0, temp, ye
  endif
#endif
...
#if 0
  if(ii.eq.3.and.jj.eq.3.and.kk.eq.3) then
     write(6,"(1P10E15.6)") udens, utau, pold, s2, utau + pold + udens
     stop "eh"
  endif
#endif
...
! DEBUG
#if 0
        write(warnline,"(A4,4i3,1P10E15.6)") "*C21", ii,jj,kk,GRHydro_Reflevel,&
             rho,epsilon,temp,ye,xpress
        call CCTK_WARN(1,warnline)
        write(warnline,"(A4,4i3,1P10E15.6)") "*C22", ii,jj,kk,GRHydro_Reflevel,&
             udens,utau,dens,tau,sdet
        call CCTK_WARN(0,warnline)
#endif
     else 
        !$OMP CRITICAL
...
!!$  Calculate primitive variables from root
  ...
  if(keyerr(1).eq.668) then
     ...
     GRHydro_C2P_failed = 3.0d0
#if 0
     write(warnline,"(A3,3i3,1P10E15.6)") "*5", ii,jj,kk,rho,epsilon,temp,ye,xpress
     call CCTK_WARN(0,warnline)
#endif
     !$OMP END CRITICAL
  endif
