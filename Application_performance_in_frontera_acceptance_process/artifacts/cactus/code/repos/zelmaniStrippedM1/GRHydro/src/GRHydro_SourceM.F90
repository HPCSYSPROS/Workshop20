...
#if(1)
#define DIFF_X_4(q) (((q(i-2,j,k)-q(i+2,j,k)) + 8.d0 * (q(i+1,j,k) - q(i-1,j,k))) / 12.d0 * idx)
#define DIFF_Y_4(q) (((q(i,j-2,k)-q(i,j+2,k)) + 8.d0 * (q(i,j+1,k) - q(i,j-1,k))) / 12.d0 * idy)
#define DIFF_Z_4(q) (((q(i,j,k-2)-q(i,j,k+2)) + 8.d0 * (q(i,j,k+1) - q(i,j,k-1))) / 12.d0 * idz)
#else
#define DIFF_X_4(q) ((-q(i+2,j,k) + 8.d0 * q(i+1,j,k) - 8.d0 * q(i-1,j,k) + q(i-2,j,k)) / 12.d0 * idx)
#define DIFF_Y_4(q) ((-q(i,j+2,k) + 8.d0 * q(i,j+1,k) - 8.d0 * q(i,j-1,k) + q(i,j-2,k)) / 12.d0 * idy)
#define DIFF_Z_4(q) ((-q(i,j,k+2) + 8.d0 * q(i,j,k+1) - 8.d0 * q(i,j,k-1) + q(i,j,k-2)) / 12.d0 * idz)
#endif
...
subroutine SourceTermsM(CCTK_ARGUMENTS)
  ...
  CCTK_REAL :: dx_psi, dy_psi, dz_psi
  CCTK_REAL :: dx_bconsx, dy_bconsx, dz_bconsx
  CCTK_REAL :: dx_bconsy, dy_bconsy, dz_bconsy
  CCTK_REAL :: dx_bconsz, dy_bconsz, dz_bconsz
...
#define Bvec faulty_Bvec
  ...
  do_evolve_Avec = CCTK_EQUALS(Bvec_evolution_method, "GRHydro_Avec")
...
!!$        Contract the matter with derivatives of the metric
       ...
       sz_source = t00*&
        ...
        if (do_evolve_Avec) then

        !! B^i and A^i both live in cell centers currently
        Avcx_source = alp(i,j,k)*sqrtdet*(velyshift*Bvecz(i,j,k) - velzshift*Bvecy(i,j,k))
        Avcy_source = alp(i,j,k)*sqrtdet*(velzshift*Bvecx(i,j,k) - velxshift*Bvecz(i,j,k))
        Avcz_source = alp(i,j,k)*sqrtdet*(velxshift*Bvecy(i,j,k) - velyshift*Bvecx(i,j,k))
        ...
        if(clean_divergence.ne.0) then
           if (local_spatial_order .eq. 2) then
             ...
             dx_psi = DIFF_X_2(psidc)
             dy_psi = DIFF_Y_2(psidc)
             dz_psi = DIFF_Z_2(psidc)
             dx_bconsx = DIFF_X_2(Bconsx)
             dy_bconsx = DIFF_Y_2(Bconsx)
             dz_bconsx = DIFF_Z_2(Bconsx)
             dx_bconsy = DIFF_X_2(Bconsy)
             dy_bconsy = DIFF_Y_2(Bconsy)
             dz_bconsy = DIFF_Z_2(Bconsy)
             dx_bconsz = DIFF_X_2(Bconsz)
             dy_bconsz = DIFF_Y_2(Bconsz)
             dz_bconsz = DIFF_Z_2(Bconsz)
   
           else
   
             dx_psi = DIFF_X_4(psidc)
             dy_psi = DIFF_Y_4(psidc)
             dz_psi = DIFF_Z_4(psidc)
             dx_bconsx = DIFF_X_4(Bconsx)
             dy_bconsx = DIFF_Y_4(Bconsx)
             dz_bconsx = DIFF_Z_4(Bconsx)
             dx_bconsy = DIFF_X_4(Bconsy)
             dy_bconsy = DIFF_Y_4(Bconsy)
             dz_bconsy = DIFF_Z_4(Bconsy)
             dx_bconsz = DIFF_X_4(Bconsz)
             dy_bconsz = DIFF_Y_4(Bconsz)
             dz_bconsz = DIFF_Z_4(Bconsz)
   
           end if

           if(divergence_flux.ne.0) then
             ...
             psidcrhs(i,j,k) = -1.0d0*kap_dc*alp(i,j,k)*psidc(i,j,k) + beta1(i,j,k)*dx_psi + beta2(i,j,k)*dy_psi + beta3(i,j,k)*dz_psi - &
                               alp(i,j,k)/sqrtdet*(dx_bconsx + dy_bconsy+dz_bconsz)
           end if
           ...
           if(divergence_flux .ne. 0) then
             ...
             bvcx_source = beta1(i,j,k)*dx_bconsx + beta1(i,j,k)*dy_bconsy + beta1(i,j,k)*dz_bconsz - &
                           alp(i,j,k)*sqrtdet*(uxx*dx_psi+uxy*dy_psi+uxz*dz_psi) 
             bvcy_source = beta2(i,j,k)*dx_bconsx + beta2(i,j,k)*dy_bconsy + beta2(i,j,k)*dz_bconsz - &
                           alp(i,j,k)*sqrtdet*(uxy*dx_psi+uyy*dy_psi+uyz*dz_psi) 
             bvcz_source = beta3(i,j,k)*dx_bconsx + beta3(i,j,k)*dy_bconsy + beta3(i,j,k)*dz_bconsz - &
                           alp(i,j,k)*sqrtdet*(uxz*dx_psi+uyz*dy_psi+uzz*dz_psi) 
           end if
