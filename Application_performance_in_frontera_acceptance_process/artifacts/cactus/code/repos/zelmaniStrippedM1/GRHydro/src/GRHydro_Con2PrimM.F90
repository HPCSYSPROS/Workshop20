...
subroutine Conservative2PrimitiveM(CCTK_ARGUMENTS)
  ...
  CCTK_REAL :: s2m, s2m0, s2m_resid, s2mold, s2max, taum, dummy1, dummy2, v2
  CCTK_INT :: niter
...
!       GRHydro_rho_min,xeps,xtemp,xye,pmin,epsmin,keyerr,anyerr)
  ...
  !$OMP w_lorentz_tmp,Bvecx_tmp,Bvecy_tmp,Bvecz_tmp,bdotv,magpress, dummy1, dummy2, v2)
  do k = 1, nz 
     do j = 1, ny 
        do i = 1, nx
           
           !! Adjust initial guess for vel to ensure v^2 <= 1!
           !! Note: since the initial guess is from the last time step, but the metric has changed in the meantime,
           !!       v2 < 1 is not guaranteed anymore. Since this is an initial guess, however, we are allowed to do some
           !!       adjustments.
           !! Note2: This must be executed before atmopshere skip below!
           v2 = g11(i,j,k)*vup(i,j,k,1)*vup(i,j,k,1) + &
                   g22(i,j,k)*vup(i,j,k,2)*vup(i,j,k,2) + &
                   g33(i,j,k)*vup(i,j,k,3)*vup(i,j,k,3) + 2.0d0 * ( &
                   g12(i,j,k)*vup(i,j,k,1)*vup(i,j,k,2) + & 
                   g13(i,j,k)*vup(i,j,k,1)*vup(i,j,k,3) + &
                   g23(i,j,k)*vup(i,j,k,2)*vup(i,j,k,3) )
           if (v2 .ge. 1.0d0) then
              vup(i,j,k,1) = vup(i,j,k,1) / (v2*(1.0d0+1.0d-7))
              vup(i,j,k,2) = vup(i,j,k,2) / (v2*(1.0d0+1.0d-7))
              vup(i,j,k,3) = vup(i,j,k,3) / (v2*(1.0d0+1.0d-7))
           end if
           if (v2 .lt. 0.0d0) then
              vup(i,j,k,1) = 0.0d0
              vup(i,j,k,2) = 0.0d0
...
!                !$OMP CRITICAL
!                if (rho(i,j,k).ne.rho(i,j,k) .or. Bvecx_tmp.ne.Bvecx_tmp .or. Bvecy_tmp.ne.Bvecy_tmp .or. Bvecz_tmp.ne.Bvecz_tmp .or. Bcons(i,j,k,1).ne.Bcons(i,j,k,1) .or. Bcons(i,j,k,2).ne.Bcons(i,j,k,2) .or. Bcons(i,j,k,3).ne.Bcons(i,j,k,3)) then
!                   call CCTK_WARN(1,"rho/Bvec/Bcons NAN right before Con2Prim_pt")
...
!                   write(warnline,"(1P10E15.6)") rho(i,j,k),dens(i,j,k),eps(i,j,k)
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(1P10E15.6)") Bvecx_tmp,Bvecy_tmp,Bvecz_tmp
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(1P10E15.6)") Bcons(i,j,k,1),Bcons(i,j,k,2),Bcons(i,j,k,3)
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(1P10E15.6)") vup(i,j,k,1),vup(i,j,k,2),vup(i,j,k,3)
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(1P10E15.6)") g11(i,j,k),g12(i,j,k),g13(i,j,k)
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(1P10E15.6)") g22(i,j,k),g23(i,j,k)
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(1P10E15.6)") g33(i,j,k)
!                   call CCTK_WARN(1,warnline)
...
!                !$OMP CRITICAL
!                if (rho(i,j,k).ne.rho(i,j,k) .or. Bvecx_tmp.ne.Bvecx_tmp .or. Bvecy_tmp.ne.Bvecy_tmp .or. Bvecz_tmp.ne.Bvecz_tmp) then
!                   call CCTK_WARN(1,"rho/Bvec NAN right after Con2Prim_pt")
!                   write(warnline,"(A10,i5)") "reflevel: ",GRHydro_reflevel
!                   call CCTK_WARN(1,warnline)
!                   write(warnline,"(3i5,1P10E15.6)") i,j,k,x(i,j,k),y(i,j,k),z(i,j,k)
!                   call CCTK_WARN(1,warnline)
...
!                !$OMP END CRITICAL
              ...
              if(GRHydro_C2P_failed(i,j,k).ne.0) then
!                 !$OMP CRITICAL
!                 call CCTK_WARN(1,"C2P_failed right after Con2Prim_pt")
!                 write(warnline,"(A10,i5)") "reflevel: ",GRHydro_reflevel
!                 call CCTK_WARN(1,warnline)
!                 write(warnline,"(3i5,1P10E15.6)") i,j,k,x(i,j,k),y(i,j,k),z(i,j,k)
!                 call CCTK_WARN(1,warnline)
!                 write(warnline,"(3i5,1P10E15.6)") GRHydro_C2P_failed(i,j,k)
!                 call CCTK_WARN(1,warnline)
...
!                   if (g11(i,j,k)*vup(i,j,k,1)*vup(i,j,k,1) + g22(i,j,k)*vup(i,j,k,2)*vup(i,j,k,2) + g33(i,j,k)*vup(i,j,k,3)*vup(i,j,k,3) + 2.0d0 * ( g12(i,j,k)*vup(i,j,k,1)*vup(i,j,k,2) + g13(i,j,k)*vup(i,j,k,1)*vup(i,j,k,3) + g23(i,j,k)*vup(i,j,k,2)*vup(i,j,k,3) ) .ge. 1.0d0) then
!                      call CCTK_WARN(0, "v2 >= 1 after C2P failed in Con2Prim!!")
!                   endif
                  cycle
                end if
              end if
...
!             if(GRHydro_C2P_failed(i,j,k).ne.0) then
!             !$OMP CRITICAL
!                 call CCTK_WARN(1,"C2P_failed right after Con2Prim_Polytype_pt")
!                 write(warnline,"(A10,i5)") "reflevel: ",GRHydro_reflevel
!                 call CCTK_WARN(1,warnline)
!                 write(warnline,"(3i5,1P10E15.6)") i,j,k,x(i,j,k),y(i,j,k),z(i,j,k)
!                 call CCTK_WARN(1,warnline)
