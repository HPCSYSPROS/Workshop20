...
/* Subroutine */ int passb2(int *ido, int *l1, double *cc, 
...
/* Subroutine */ int passb3(int *ido, int *l1, double *cc, 
	double *ch, double *wa1, double *wa2)
{
    ...
    /* Local variables */
    static int i, k;
    static double ci2, ci3, di2, di3, cr2, cr3, dr2, dr3, ti2, tr2;
...
/* Subroutine */ int passf3(int *ido, int *l1, double *cc, 
	double *ch, double *wa1, double *wa2)
{
    /* Initialized data */
...
/* Subroutine */ int passf4(int *ido, int *l1, double *cc, 
	double *ch, double *wa1, double *wa2, double *wa3)
{
    /* System generated locals */
    int cc_dim1, cc_offset, ch_dim1, ch_dim2, ch_offset, i_1, i_2;
...
/* Subroutine */ int passf5(int *ido, int *l1, double *cc, 
	double *ch, double *wa1, double *wa2, double *wa3, 
	double *wa4)
{
    /* Initialized data */
...
/* Subroutine */ int cfftf1(int *n, double *c, double *ch, 
	double *wa, int *ifac)
{
    /* System generated locals */
    int i_1;
...
/* L70: */
	    ...
	    if (*isign == -1) {
		cfftf(n1, (double*) &work[1], &table[table_dim1 + 1]);
	    }
	    if (*isign == 1) {
		cfftb(n1, (double*) &work[1], &table[table_dim1 + 1]);
	    }
	    i_3 = *n1;
	    for (i = 1; i <= i_3; ++i) {
...
/* L170: */
	    ...
	    if (*isign == -1) {
		cfftf(n2, (double*) &work[1], &table[(table_dim1 << 1) + 1]);
	    }
	    if (*isign == 1) {
		cfftb(n2, (double*) &work[1], &table[(table_dim1 << 1) + 1]);
	    }
	    i_3 = *n2;
	    for (j = 1; j <= i_3; ++j) {
		i_4 = i + (j + k * w_dim2) * w_dim1;
...
/* L270: */
	    ...
	    if (*isign == -1) {
		cfftf(n3, (double*) &work[1], &table[table_dim1 * 3 + 1]);
	    }
	    if (*isign == 1) {
		cfftb(n3, (double*) &work[1], &table[table_dim1 * 3 + 1]);
	    }
	    i_3 = *n3;
	    for (k = 1; k <= i_3; ++k) {
		i_4 = i + (j + k * w_dim2) * w_dim1;
...
int pubdz3d(int isign, int n1, int n2,
  ...
  double *data, *data2;
  double TwoPiOverN, tmp1r, tmp1i, tmp2r, tmp2i;

  /* complex transform */
  n1over2 = n1 / 2;
  ld1over2 = ld1 / 2;
  rval = pubz3d(&isign, &n1over2, &n2, &n3, (doublecomplex*) w,
         &ld1over2, &ld2, table, &ntable, (doublecomplex*) work);
...
int pubzd3d(int isign, int n1, int n2,
  ...
  n1over2 = n1 / 2;
  ld1over2 = ld1 / 2;
  return pubz3d(&isign, &n1over2, &n2, &n3, (doublecomplex*) w,
         &ld1over2, &ld2, table, &ntable, (doublecomplex*) work);
