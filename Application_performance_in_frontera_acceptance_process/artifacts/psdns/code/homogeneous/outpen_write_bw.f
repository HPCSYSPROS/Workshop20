      subroutine outpen_write_bw (buf,iwgid)
c
...
c
      ...
      data icall/0/
c
	if (taskid.eq.0) write (6,*) 'enter outpen_write_bw'
...
c
	...
	if (taskid.eq.0) write (6,*) 'icall, fname=',icall,fname
c
...
c
	...
	if(i.eq.2) then
!      write (lu) ((ureal(xp,y),uimag(xp,y),xp=1,xisz),y=1)
      write (lu) (ureal(xp,1),uimag(xp,1),xp=1,xisz)
	else
