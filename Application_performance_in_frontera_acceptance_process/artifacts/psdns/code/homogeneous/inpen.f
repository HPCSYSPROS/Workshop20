...
c
	...
	integer no_err
c
c 3 lines below added to allow operation with io_scheme=2,
c corresponding to suggestion from Mike Matheson, Aug 2012
c
	integer itask,msgtag,ncount,xi1,zj1,io_scheme,mread0
...
c       if (bwin.ne.0) call MPI_BCAST (bwin,1,MPI_INTEGER,0,MPI_COMM_WORLD,ierr)
...
c
	if (taskid.eq.0) then
        ...
        call MPI_BCAST (ind_prec,1,MPI_INTEGER,0,MPI_COMM_WORLD,mpierr)
...
c
	...
	write (6,*) 'io_scheme=',io_scheme,mread0
	end if
...
c
 	...
 	if (mod(numtasks,262144).eq.0) mread=8192
...
c
	...
	if (relay.eq.2) write (6,*) 'relay signal received, taskid,relay=',taskid,relay
#ifdef READR
           if (bwin.eq.1) then
           call inpen_read_bw (uny,slabpenc,iproc0,ixfst,ixlst,jzfst,jzlst,ndirs,ind_prec,relay,xist,zjst,taskid)
           else
	...
	if (iread.lt.nread-1) then
     ...
     1                MPI_COMM_WORLD,mpierr)
	if (relay.eq.2) write (6,*) 'relay signal   sent  , taskid,relay=',taskid,relay
	end if
c
!----------------------------------------------------
...
c	
	...
	ncount=ny*zjsz*xisz*3
	if (nc.gt.0.and.kinit(3+nc).gt.0) ncount=ncount*(3+nc)/3
c
	if (ipid.eq.0) then
c
	do itask=taskid+iproc-1,taskid,-1
	msgtag=taskid*10000+itask
	xi1=xist_all(itask)
	zj1=zjst_all(itask)

           if (bwin.eq.1) then
           call inpen_read_bw (uny,slabpenc,iproc0,ixfst,ixlst,jzfst,jzlst,ndirs,ind_prec,relay,xi1,zj1,itask)
           else
           call inpen_read_r (uny,slabpenc,iproc0,ixfst,ixlst,jzfst,jzlst,ndirs,ind_prec,relay,xi1,zj1,itask)
           endif

		if (itask.gt.taskid) then
           call MPI_SSEND (uny,ncount,mpicomplex,itask,msgtag,
     1                MPI_COMM_WORLD,mpierr)
		end if
	end do
