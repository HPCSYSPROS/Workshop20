...
c
	allocate (pptmp(nop2/numtasks,j1:j2))
	do j=j1,j2
...
c
c	if (taskid.eq.0.and.jstep.eq.1) write (6,*) 'enter pop2_write5, istep,j1,j2=',istep,j1,j2
	if (taskid.eq.0) write (6,*) 'enter pop2_write5, istep,j1,j2=',istep,j1,j2
c
...
c
        ...
        jtask=taskid-dest
        msgtype(imt1,jtask)=1000*imt1+jtask
        call MPI_ISEND (pptmp(1,j),mp,pptype,dest,
     1                  msgtype(imt1,jtask),
