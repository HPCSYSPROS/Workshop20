...
c	call ppminmax ('rk_part2',pp2,ndpart2)
	do ir=1,nrec2

	indx = (ir-1)*nprec/numtasks + 1

c        call intbf_ars ( xyzl,nxyz,gsh(1,1),pp2(1,1),indx,nrec2,ndpart2,
c     1               bfpos,slant, 2)

#ifdef CF_LAG
c       call spcal_tree ( bfpos, bs(1,1,1,1),  pp2(1,4), 3, ps(1),
c    1             indx, nrec2, ndpart2, iod(1,1))
	call spcal_pipe ( bs(1,1,1,1), pp2(1,1), 4, 3, ndprop2, sf,
     1                    ndpart2, iod(1,1), 2 )
c
#else
...
c
	...
	if (taskid.eq.0) then
	write (6,"('cpu_rkpart:',2i3,1p,3e12.4)") istep,jj,sum(cpu_intbf(4,1:2,jj)),sum(cpu_spcal(4,1:2,jj)),cpu_rkpart(2,jj)
	end if
	end if
