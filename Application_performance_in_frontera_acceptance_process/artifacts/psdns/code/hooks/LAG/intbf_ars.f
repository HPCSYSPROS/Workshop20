...
c
	real(p8) px,xyzl8,px1,px2,px3,px4
c
      integer nxyz(3)
      real(b8) xyzl(3),shif(3)
...
c
 	call ppminmax ('enter intbf_ars',xp,npdim)
...
!$OMP PARALLEL private (ithr, i,j,m,ip,px,ipx,jpx,ploc,jloc,
!$OMP& px1,px2,px3,px4,mp1,mp2,ipmax) shared (nbad_thr)
!$OMP& REDUCTION (+:nbad)
#ifdef OPENMP
...
!cc        ipx=px
! fix by PKY on 6/28/2014
        ipx=floor(px)
        jpx=ipx
!        if (ipx.lt.0) jpx=ipx+100*nxyz(j)
        if (ipx.lt.0) jpx=ipx+1000*nxyz(j)
        jloc=mod(jpx,nxyz(j))
!        bfpm(j,i)=jloc+px-ipx
        bfp(j,i)=real(jloc)+px-real(ipx)


!	if(icls.eq.1) then
!	write(7000+taskid,*) i,m,j,xp(ip,j),nxyz(j),shif(j),gfn(j),px1,px2,px3,px4,px,bfp(j,i)
!	endif
