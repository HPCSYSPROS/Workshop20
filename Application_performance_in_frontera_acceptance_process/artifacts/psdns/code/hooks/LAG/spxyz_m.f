...
c 2D code
c
c CHanges by D. Buaria in early March 2015: allocate
c memory for temporary arrays used by splx,sply,splz here.
c also initialize bsx and buf1 to zero immediately after allocation
...
c
      ...
      real(p8)  bsy(bxisize,nby,bzjsize,nv)
       real(p8), allocatable :: uth(:,:),vth(:,:),wth(:,:)
...
c
        nsz = max(nx,ny,nz)
        allocate (uth(0:nsz,0:num_thr-1))
        allocate (vth(0:nsz,0:num_thr-1))
        allocate (wth(0:nsz,0:num_thr-1))
...
c
	...
	ithr=0
c!$OMP PARALLEL private (ithr,pos1)
!$OMP PARALLEL private (ithr,pos1,ii)
#ifdef OPENMP
...
c         call splx (f(1,1,y,iv),bsx(1,1,y,iv))
          call splx (f(1,1,y,iv),bsx(1,1,y,iv),uth(0,ithr),vth(0,ithr))
 10   continue
...
c         call splz_m (bsz(1,1,1,yp),nv)
         call splz_m (bsz(1,1,1,yp),nv,uth(0,ithr),vth(0,ithr),wth(0,ithr))
	end do
...
c         do zp=1,bzjsize
	do zp=zpp1(ithr),zpp1(ithr)+zppi(ithr)-1
c            call sply (bsy(1,1,zp,iv))
           call sply (bsy(1,1,zp,iv),uth(0,ithr),vth(0,ithr))
      enddo
      enddo
