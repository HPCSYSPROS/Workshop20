      subroutine spcal_ars ( bfp,bsxy,f,nv,sf,n1,nrec,npdim,iod, ityp)
...
c
c Operation with OMP seems safer if the 'bff' and 'swork1' arrays
c are allocated and deallocated outside of parallel region,  and
c declared as private variables
c
...
c
      ...
      real(p8), allocatable :: work(:,:),fg(:),swork1(:)
...
c
        ...
        cpuspcal=0.
c
      allocate (work(nprec,nv),fg(nprec),stat=ierr)
        if (ierr.gt.0) then
        write (6,*) 'spcal_ars: error at allocate work and fg'
        end if
c
	end if
