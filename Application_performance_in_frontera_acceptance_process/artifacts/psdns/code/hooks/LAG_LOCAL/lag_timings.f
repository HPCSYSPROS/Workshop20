...
! use nspcal for timings per step since intbf no longer used for nop and nop2
	cpu_intbf(:,:,1)=cpu_intbf(:,:,1)/nspcal
...
c
 ...
 645    format (i6,2i5,1x,1p,8e10.3)
	...
	write(7,"('nop = ',i9,'  nop2 =',i9,'  nom = ',i9)")nop,nop2,nom
        write (7,"('--------------------------------------------')")
	write(7,"('taskid ipid jpid    spxyz  interpoltn    total')")
	do itask=0,numtasks-1
	if (itask.le.iproc-1.or.itask.ge.numtasks-iproc) then
