      subroutine ppminmaxv (string,xp,npdim,nump)
c
#ifdef LAG
...
c somehow gone far out of reasonable range
	...
	implicit none
c
	integer i,j,itask,npdim, nump
	real(p8) pmin,pmax,pmin0,pmax0
	character*(*) string
	real(p8) xp(nppad*npdim/numtasks,3)
	real(p8) pmin_all(0:numtasks-1),pmax_all(0:numtasks-1)
c
 	return

	if (npdim.eq.0) then
	if(taskid.eq.0) write(6,*) 'ppminmaxv, npdim is 0'
	if(taskid.eq.numtasks-1) write(6,*) 'ppminmaxv, npdim is 0'
	return
	endif
