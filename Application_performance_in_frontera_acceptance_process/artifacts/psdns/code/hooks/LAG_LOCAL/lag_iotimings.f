...
c
	...
	allocate (t_lagout_all(0:numtasks-1))
     ...
     1                       MPI_COMM_WORLD,mpierr)
	cpu_spxyz(:,2)=cpu_spxyz(:,2)/(nspcal_io/2)
        call  MPI_ALLGATHER (cpu_spxyz(0,2),11,MPI_DOUBLE_PRECISION,
     1                       t_spxyz_all,11,MPI_DOUBLE_PRECISION,
     1                       MPI_COMM_WORLD,mpierr)
	cpu_partic2(2)=cpu_partic2(2)/(nspcal_io/2)
        call  MPI_ALLGATHER (cpu_partic2(2),1,MPI_DOUBLE_PRECISION,
     1                       t_partic2_all,1,MPI_DOUBLE_PRECISION,
     1                       MPI_COMM_WORLD,mpierr)
	cpu_partsp(:,2)=cpu_partsp(:,2)/(nspcal_io/2)
        call  MPI_ALLGATHER (cpu_partsp(0,2),6,MPI_DOUBLE_PRECISION,
     1                       t_partsp_all,6,MPI_DOUBLE_PRECISION,
     1                       MPI_COMM_WORLD,mpierr)
	cpu_rkpart(:,2)=cpu_rkpart(:,2)/(nspcal_io/2)
        call  MPI_ALLGATHER (cpu_rkpart(1,2),2,MPI_DOUBLE_PRECISION,
     1                       t_rkpart_all,2,MPI_DOUBLE_PRECISION,
     1                       MPI_COMM_WORLD,mpierr)
! Aug 17, 2015, D. Buaria
! seems nintbf instead of nintbf_io was incorrectly used here
! In any case changed to nspcal_io as done in lag_timings.f
!	cpu_intbf(:,:,2)=cpu_intbf(:,:,2)/nintbf
	cpu_intbf(:,:,2)=cpu_intbf(:,:,2)/nspcal_io
        call  MPI_ALLGATHER (cpu_intbf(1,1,2),4*3,MPI_DOUBLE_PRECISION,
     1                       t_intbf_all,4*3,MPI_DOUBLE_PRECISION,
     1                       MPI_COMM_WORLD,mpierr)
	cpu_spcal(:,:,2)=cpu_spcal(:,:,2)/nspcal_io
	if (taskid.eq.0) write (6,*) 'nintbf_io,nspcal_io=',nintbf_io,nspcal_io
        call  MPI_ALLGATHER (cpu_spcal(1,1,2),4*3,MPI_DOUBLE_PRECISION,
...
c
 ...
 711  format ('Pencils code, nx,ny,nz,nc=',3i6,i3,'  iproc, jproc=',i4,
	...
	write (7,"('averaged over', i4,'  output steps')") ncpusteps_io
...
c
	...
	open(unit=7,file=dir//'t_spxyz2_all')
      write (7,711) nx,ny,nz,nc,iproc, jproc
        write (7,"('num_thr=',i3,'  single prec,  rkmethod=',i3)") num_thr,
     1    rkmethod
	write(7,"('nop = ',i7,'  nop2 =',i7)")nop,nop2
        write (7,"('--------------------------------------------')")
 ...
 632    format (i6,2i5,1x,1p,7e10.3)
	...
	open(unit=7,file=dir//'t_intbf_all')
      write (7,711) nx,ny,nz,nc,iproc, jproc
        write (7,"('num_thr=',i3,'  single prec,  rkmethod=',i3)") num_thr,
     1    rkmethod
	write(7,"('nop = ',i7,'  nop2 =',i7,'  nom = ',i8)")nop,nop2,nom
        write (7,"('--------------------------------------------')")
