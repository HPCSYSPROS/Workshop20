      subroutine spcal_caf ( bfp,bsxy,f,nv,sf,n1,nrec,npdim,iod)
...
c
        ...
        if(taskid.eq.0) write(6,*) 'icallsp=',icallsp
...
c
	sbb=-1
        ...
        if (ierr.gt.0) then
        write (6,*) 'spcal_caf: error at allocate work'
        end if
c
	end if
...
c
	work(:,:)=0.
	call divide_thr (nprec, bp1, bpi, 'spcal_caf: nprec')
...
c
        do 200 itask=0,numtasks-1
        
        if(rantasks(itask).eq.taskid) goto 200

        if (sbb.lt.0) then
        temp(:) = work(mp1:mp2,iv)[rantasks(itask)]
        f(ip1:ip2,iv) = f(ip1:ip2,iv) + temp(:)
        else
        do ii=1,np/sbb
        i1=(ii-1)*sbb + 1
        i2=i1 + sbb - 1
        temp(:) = work(mp1+i1-1:mp1+i2-1,iv)[rantasks(itask)]
        f(ip1+i1-1:ip1+i2-1,iv)=f(ip1+i1-1:ip1+i2-1,iv) + temp(:)
        enddo
        endif

!       f(ip1:ip2,iv)=f(ip1:ip2,iv)+work(mp1:mp2,iv)[rantasks(itask)]
