...
! Set the size of the coarray buffer used during the transfers
      ...
      subroutine init_compi_alltoall()
...
! WARNING: PSDNS ALSO SETS ITS OWN RANDOM SEEDS; THERE COULD BE SIDE EFFETS!
      ...
      first=0
      end subroutine init_compi_alltoall
...
!       sync all
      ...
      end subroutine compi_alltoall




      subroutine compi_alltoallv(src,scount,sdispls,maxscount,
...
!       sync all
      return
      end subroutine compi_alltoallv

#elif defined (CAFI) || defined (CAFI_TEST)
...
! Set the stripmine blocksize in bytes
      ...
      integer, save :: first=1,if_row=1,if_col=1
      integer, allocatable :: myimrow(:),immap_row(:),rlist_row(:)
      integer, allocatable :: myimcol(:),immap_col(:),rlist_col(:)
#ifdef CAFI
      complex(b8), allocatable :: sendbuf(:)[:]
#endif
#ifdef CAFI_TEST
      complex(b8), allocatable :: cafsbuf(:)[:]
#endif
      real(b8) :: xrand
      end module
...
! WARNING: PSDNS ALSO SETS ITS OWN RANDOM SEEDS; THERE COULD BE SIDE EFFETS!
      ...
      if (first.ne.1) print *,"ERROR: init_compi_alltoall called incorrectly"
      first=0
      call random_seed(size=seedsize)    
      allocate(seed(seedsize))
      do i=1,seedsize
      ...
      call random_seed(put=seed)      
!
! Allocate a single large coarray only once
!
#if defined (CAFI) || defined (CAFI_TEST)
      nvb = nc+6
      coisz = max(KrCntMax*yjsz,IfCntMax)*nvb*iproc/(b8*2)
      coisz = max(coisz,jjsz(0)*num_al_i(0)*nvb*jproc)
      call mpi_allreduce(coisz,comxsz,1,mpi_integer,mpi_max,mpi_comm_world,ierr)
!      print *,'Rank, KrCntMax, yjszm, IfCntMax, jj, na, comxsz = ',
!     & taskid, KrCntMax, yjsz, IfCntMax, jjsz(0), num_al_i(0), comxsz
#ifdef CAFI
      allocate (sendbuf(1:comxsz)[0:*])
#endif
#ifdef CAFI_TEST
      allocate (cafsbuf(1:comxsz)[0:*])
#endif
#endif
      end subroutine init_compi_alltoall
      ...
      if_col=0
      allocate (myimcol(0:jproc-1))
      myimcol=this_image()-1
      allocate(immap_col(0:jproc-1))
      allocate(rlist_col(0:jproc-1))
      call mpi_alltoall (myimcol,1, mpi_integer,
     &        immap_col,1,mpi_integer,mpi_comm_col,ierr)
!      print *,"taskid immap_col",taskid,immap_col(0:jproc-1)
      do i=0,jproc-1
        rlist_col(i)=i
      enddo
      do i=0,jproc-1
        call random_number(xrand)
        i1=max(0,min(floor(xrand*jproc),jproc-1))
        i2=rlist_col(i1)
        rlist_col(i1)=rlist_col(i)
        rlist_col(i)=i2
      enddo
!      print *,"taskid rlist_col",taskid,rlist_col(0:jproc-1)
      end subroutine init_compi_col
      ...
      if_row=0
      allocate (myimrow(0:iproc-1))
      myimrow=this_image()-1
      allocate(immap_row(0:iproc-1))
      allocate(rlist_row(0:iproc-1))
      call mpi_alltoall (myimrow,1, mpi_integer,
     &        immap_row,1,mpi_integer,mpi_comm_row,ierr)
      do i=0,iproc-1
        rlist_row(i)=i
      enddo
      do i=0,iproc-1
        call random_number(xrand)
        i1=max(0,min(floor(xrand*iproc),iproc-1))
        i2=rlist_row(i1)
        rlist_row(i1)=rlist_row(i)
        rlist_row(i)=i2
      enddo
      end subroutine init_compi_row
#else
      subroutine compi_alltoall
! Dummy routine
      return
      end subroutine compi_alltoall
      subroutine compi_alltoallv
! Dummy routine
      return
      end subroutine compi_alltoallv
#endif
