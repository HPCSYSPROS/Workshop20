...
begin_initialization {
  ...
  // Sanity Check - How much free memory do we have?

  //uint64_t free_memory = SystemRAM::available()/1000;
  //if (rank() == 0) MESSAGE((" ******** Checking free Memory ********"));
  //if ( free_memory < 7500) {
  //  MESSAGE((" *** Warning --> Low Memory on rank = %g   free = %ld", rank(),free_memory));
  // }
  //else {
  //  MESSAGE((" *** Available Memory on rank = %g   free = %ld", rank(),free_memory));
  // }
 ...
 double eps0 = 1;         // Permittivity of space
  ...
  double nb_n0   = 0.30;   // background plasma density
  double wpe_wce = 3;      // electron plasma freq / electron cyclotron freq
  double bg = 1.0;        // electron plasma freq / electron cyclotron freq
  double theta   = 0;      // B0 = Bx
  double taui    = 20;    // simulation wci's to run

  double quota   = 10.6;   // run quota in hours
  ...
  double Ly            = 300*di;  // size of box in y dimension
  double Lz            = 50*di; // size of box in z dimension

  double topology_x = 24;  // Number of domains in x, y, and z
  double topology_y = 24;
  ...
  double uthi =  sqrt(Ti/mi)/c;              // normalized ion thermal velocity
  double uthe =  sqrt(Te/me)/c;              //  normalized electron thermal velocity
  double utheb = sqrt(Te/me)/c;             // normalized background electron thermal velocity
  double uthib = sqrt(Ti/mi)/c;             // normalized background ion thermal velocity
  double tanhf = tanh(0.5*Lz/L);
  double Lpert = 1.5*Lx;   // wavelength of perturbation
  double dbz =  0.0*b0; //  Perturbation in Bz relative to Bo (Only change here)
...
//     		    (x-0.5*Lx)*PERT ); // Bz
  ...
  do {
   ...
   // node and will not inject particle locally
    ...
    ux = maxwellian_rand(uthe);
    uy = maxwellian_rand(uthe);
    uz = maxwellian_rand(uthe);
    d0 = gdre*uy + sqrt(ux*ux + uy*uy + uz*uz + 1)*udre;
    uy = d0*cs - ux*sn;
    ux = d0*sn + ux*cs;
    inject_particle(electron, x, y, z, ux, uy, uz, qe_s, 0, 0 );

    ux = maxwellian_rand(uthi);
    uy = maxwellian_rand(uthi);
    uz = maxwellian_rand(uthi);
  ...
  inject_particle( electron, x, y, z,
                       maxwellian_rand(utheb),
                       maxwellian_rand(utheb),
                       maxwellian_rand(utheb),qe_b, 0, 0);

  inject_particle( ion, x, y, z,
                       maxwellian_rand(uthib),
                       maxwellian_rand(uthib),
                       maxwellian_rand(uthib),qi_b, 0 ,0 );
  }
  ...
  sim_log( "Finished loading particles" );
	...
	*/
	...
	global->hedParams.output_variables( current_density | charge_density | ke_density );
	global->hHdParams.output_variables( current_density | charge_density | ke_density );
